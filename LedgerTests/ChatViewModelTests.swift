...
final class ChatViewModelTests: XCTestCase {
    ...
    func testPersistsStructuredToolOutputs() async throws {
        ...
        let profiles = try TestHelpers.fetchAll(ProfileEntry.self, from: context)
        ...
        XCTAssertEqual(profiles.first?.key, "goal")
        XCTAssertEqual(profiles.first?.value, "cut")
        let completedToolUseCount = await client.completedToolUseCount()
        XCTAssertEqual(completedToolUseCount, 4)
    }

    func testUpsertsProfileEntriesAcrossSends() async throws {
        let container = try TestHelpers.makeInMemoryContainer()
        let context = ModelContext(container)
        let client = StubStreamingClient(
            scripts: [
        ...
        let profiles = try TestHelpers.fetchAll(ProfileEntry.self, from: context)
        XCTAssertEqual(profiles.count, 1)
        XCTAssertEqual(profiles.first?.value, "maintain")
    }
